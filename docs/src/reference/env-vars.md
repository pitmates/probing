...
| `PROBING_TORCH_ROOFLINE_CONFIG` | 鈥?| Consolidated roofline tuning config: inline JSON or a path to a JSON file. Keys: `backend` (`auto`/`cuda`/`rocm`), `rocm_enabled`, `rocprof_cmd`, `probe_cmd`, `metrics`, `peaks`, `flop_weights`. Prefer this over the legacy per-vendor env vars below. |
